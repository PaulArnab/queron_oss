...
def resolve_connection_binding(binding_name: str, config: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw_type in {"postgres", "postgresql"}:
        ...
        resolved["type"] = "db2"
    elif raw_type in {"mssql", "sqlserver", "sql_server"}:
        resolved["type"] = "mssql"
    elif raw_type == "mysql":
        resolved["type"] = "mysql"
    elif raw_type == "mariadb":
        resolved["type"] = "mariadb"
    elif raw_type == "oracle":
        resolved["type"] = "oracle"
    else:
        raise RuntimeError(
            f"Connection '{binding_name}' must declare a supported type ('postgres', 'db2', 'mssql', 'mysql', 'mariadb', or 'oracle')."
        )
