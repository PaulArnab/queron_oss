...
MariaDB Core
==========
MariaDB-specific connection, ingress, egress, and lookup-table cleanup helpers.
"""
...
MariaDbConnectRequest = _base_models.MariaDbConnectRequest
...
_VALID_MARIADB_AUTH_MODES = {"basic", "tls", "mtls", "socket"}
_DUCKDB_VARCHAR_RE = re.compile(r"^VARCHAR\((\d+)\)$", re.IGNORECASE)
_MARIADB_MAX_INLINE_VARCHAR_LENGTH = 16383
_MARIADB_MIN_MEASURED_VARCHAR_LENGTH = 32


class _MappedMariaDbColumn:
...
def _require_driver() -> None:
    if mariadb is None:
        raise RuntimeError("MariaDB is not installed. Install the 'mariadb' package first.")
...
def _split_target_table_name(identifier: str) -> tuple[str | None, str]:
    ...
    raise RuntimeError(f"Invalid MariaDB target table name '{identifier}'.")
...
def _infer_mariadb_auth_mode(req: MariaDbConnectRequest) -> str:
    explicit = str(getattr(req, "auth_mode", "") or "").strip().lower()
    if explicit:
        if explicit not in _VALID_MARIADB_AUTH_MODES:
            raise RuntimeError(f"Unsupported MariaDB auth_mode '{req.auth_mode}'.")
        return explicit
...
def _validate_mariadb_auth_request(req: MariaDbConnectRequest, auth_mode: str) -> None:
    if auth_mode not in _VALID_MARIADB_AUTH_MODES:
        raise RuntimeError(f"Unsupported MariaDB auth_mode '{auth_mode}'.")
    if auth_mode == "socket" and not str(getattr(req, "unix_socket", "") or "").strip():
        raise RuntimeError("MariaDB socket auth requires unix_socket.")
    if auth_mode in {"tls", "mtls"} and not str(getattr(req, "ssl_ca", "") or "").strip():
        raise RuntimeError(f"MariaDB {auth_mode} auth requires ssl_ca.")
    if auth_mode == "mtls":
        if not str(getattr(req, "ssl_cert", "") or "").strip() or not str(getattr(req, "ssl_key", "") or "").strip():
            raise RuntimeError("MariaDB mTLS auth requires ssl_cert and ssl_key.")


def _resolved_mariadb_connect_config(req: MariaDbConnectRequest) -> dict[str, Any]:
    auth_mode = _infer_mariadb_auth_mode(req)
    _validate_mariadb_auth_request(req, auth_mode)
    cfg: dict[str, Any] = {
        "connection_id": str(getattr(req, "connection_id", "") or "").strip() or uuid.uuid4().hex,
        "name": str(getattr(req, "name", "") or "").strip() or "mariadb",
    ...
    if auth_mode in {"tls", "mtls"}:
        cfg["ssl_ca"] = str(getattr(req, "ssl_ca", "") or "").strip()
        if auth_mode == "mtls":
            cfg["ssl_cert"] = str(getattr(req, "ssl_cert", "") or "").strip()
            cfg["ssl_key"] = str(getattr(req, "ssl_key", "") or "").strip()
    return cfg


def _connect_from_request(req: MariaDbConnectRequest):
    _require_driver()
    cfg = _resolved_mariadb_connect_config(req)
    kwargs: dict[str, Any] = {
        "host": cfg["host"],
        "port": int(cfg["port"]),
        "user": cfg["username"],
    ...
    if cfg["ssl_cert"]:
        kwargs["ssl_cert"] = cfg["ssl_cert"]
    if cfg["ssl_key"]:
        kwargs["ssl_key"] = cfg["ssl_key"]
...
def _normalize_mariadb_source_type(
    type_code: Any,
    *,
    ...
    if normalized in {"DECIMAL", "NEWDECIMAL"}:
        ...
        if resolved_precision > 38:
            warnings.append(f"MariaDB DECIMAL({resolved_precision},{resolved_scale}) exceeds DuckDB max precision 38 and was widened to DOUBLE.")
    ...
    if normalized in {"JSON"}:
        warnings.append("MariaDB JSON was widened to DuckDB VARCHAR.")
        return "VARCHAR", warnings, True
    if normalized in {"VAR_STRING", "STRING", "VARCHAR", "ENUM", "SET"}:
        return "VARCHAR", warnings, False
    warnings.append(f"MariaDB type '{raw}' was widened to DuckDB VARCHAR.")
    return "VARCHAR", warnings, True


def _map_description_to_columns(description: Any) -> list[_MappedMariaDbColumn]:
    mapped: list[_MappedMariaDbColumn] = []
    for item in list(description or []):
        ...
        target_type, warnings, lossy = _normalize_mariadb_source_type(
            type_code,
            internal_size=internal_size,
            precision=precision,
            scale=scale,
        )
        mapped.append(_MappedMariaDbColumn(name=name, source_type=_field_type_name(type_code), target_type=target_type, warnings=warnings, lossy=lossy))
    return mapped


def _build_duckdb_create_table_sql(target_table: str, mapped_columns: list[_MappedMariaDbColumn], *, replace: bool = False) -> str:
...
def _is_mariadb_string_like_duckdb_column(column: ColumnMeta) -> bool:
    normalized = str(column.data_type or "UNKNOWN").strip().upper()
    return normalized == "VARCHAR" or normalized.startswith("VARCHAR(")


def _mariadb_varchar_type_for_column(column: ColumnMeta) -> tuple[str, list[str]]:
    normalized = str(column.data_type or "UNKNOWN").strip().upper()
    warnings: list[str] = []
    if isinstance(column.max_length, int) and column.max_length >= 0:
        observed_length = int(column.max_length)
        if observed_length > _MARIADB_MAX_INLINE_VARCHAR_LENGTH:
            warnings.append(
                f"DuckDB VARCHAR observed length {observed_length} exceeds MariaDB inline VARCHAR limit "
                f"{_MARIADB_MAX_INLINE_VARCHAR_LENGTH}; using LONGTEXT."
            )
            return "LONGTEXT", warnings
        padded = max(observed_length * 2, _MARIADB_MIN_MEASURED_VARCHAR_LENGTH)
        return f"VARCHAR({min(padded, _MARIADB_MAX_INLINE_VARCHAR_LENGTH)})", warnings
    varchar_match = _DUCKDB_VARCHAR_RE.match(normalized)
    if varchar_match:
        declared_length = int(varchar_match.group(1))
        if declared_length > _MARIADB_MAX_INLINE_VARCHAR_LENGTH:
            warnings.append(
                f"DuckDB VARCHAR({declared_length}) exceeds MariaDB inline VARCHAR limit "
                f"{_MARIADB_MAX_INLINE_VARCHAR_LENGTH}; using LONGTEXT."
            )
            return "LONGTEXT", warnings
        return f"VARCHAR({max(declared_length, 1)})", warnings
    warnings.append("DuckDB VARCHAR did not have measured length metadata; using MariaDB LONGTEXT.")
    return "LONGTEXT", warnings


def _measure_mariadb_string_column_lengths(
    duck_conn: Any,
    source_sql: str,
    ...
    *,
    ...
    string_columns = [column for column in columns if _is_mariadb_string_like_duckdb_column(column)]
    ...
    try:
        ...
        return [f"MariaDB egress VARCHAR length measurement failed; using LONGTEXT. {exc}"]
    if row is None:
        return []
    for index, column in enumerate(string_columns):
...
def _map_duckdb_column_to_mariadb(column: ColumnMeta) -> tuple[str, list[str]]:
    normalized = str(column.data_type or "UNKNOWN").strip().upper()
    warnings: list[str] = []
    ...
    if normalized in {"TIMESTAMP", "TIMESTAMPTZ"}:
        if normalized == "TIMESTAMPTZ":
            warnings.append("DuckDB TIMESTAMPTZ was normalized to MariaDB DATETIME.")
    ...
    if normalized == "VARCHAR" or normalized.startswith("VARCHAR"):
        target_type, length_warnings = _mariadb_varchar_type_for_column(column)
        warnings.extend(length_warnings)
        return target_type, warnings
    warnings.append(f"DuckDB type '{normalized}' was widened to MariaDB LONGTEXT.")
    return "LONGTEXT", warnings


def _build_mariadb_create_table_sql(target_table: str, columns: list[ColumnMeta]) -> tuple[str, list[str]]:
    warnings: list[str] = []
    column_defs: list[str] = []
    for column in columns:
        target_type, column_warnings = _map_duckdb_column_to_mariadb(column)
...
def _mariadb_table_exists(cur: Any, *, schema_name: str | None, table_name: str) -> bool:
    if schema_name:
        cur.execute(
...
def _inspect_mariadb_table_columns(cur: Any, *, schema_name: str | None, table_name: str) -> list[ColumnMeta]:
    if schema_name:
        cur.execute(
            """
    ...
    *,
    ...
    for index, source_column in enumerate(source_columns, start=1):
        ...
        if remote_column is None:
            warnings.append("Remote target column was not found during MariaDB schema inspection.")
        mappings.append(
            ColumnMappingRecord(
                ordinal_position=index,
...
def _ensure_mariadb_schema(cur: Any, schema_name: str | None) -> None:
...
def _build_column_mappings(mapped_columns: list[_MappedMariaDbColumn]) -> list[ColumnMappingRecord]:
    return [
        ColumnMappingRecord(
...
def _collect_mapping_warnings(mapped_columns: list[_MappedMariaDbColumn]) -> list[str]:
    warnings: list[str] = []
    seen: set[str] = set()
...
def _build_mariadb_interruptor(conn: Any) -> Any:
    def _interrupt() -> None:
        try:
            conn.close()
        except Exception:
...
def test_connection(req: MariaDbConnectRequest) -> TestConnectionResponse:
    conn = None
    try:
        ...
        return TestConnectionResponse(success=True, message=f"Connected to MariaDB database '{db_name}'.")
    except Exception as exc:
...
def connect(req: MariaDbConnectRequest) -> ConnectResponse:
    cfg = _resolved_mariadb_connect_config(req)
    conn = _connect_from_request(req)
    _connections[cfg["connection_id"]] = {"connection": conn, "config": cfg, "opened_at": time.time()}
    return ConnectResponse(connection_id=str(cfg["connection_id"]), message=f"Connected to MariaDB database '{cfg['database']}'.")
...
def run_query(connection_id: str, sql: str) -> QueryResponse:
    ...
    if not entry:
        raise RuntimeError(f"MariaDB connection '{connection_id}' is not open.")
    conn = entry.get("connection")
    cur = conn.cursor()
    ...
    *,
    ...
    try:
        ...
        if callable(on_interrupt_open):
            source_interrupt_token = on_interrupt_open(_build_mariadb_interruptor(source_conn))
...
def egress_query_from_duckdb(
    *,
    target_request: MariaDbConnectRequest,
    duckdb_database: str,
    sql: str,
    sql_params: list[Any] | None = None,
    target_table: str,
    ...
    if normalized_mode not in {"replace", "append", "create", "create_append"}:
        raise RuntimeError(f"Unsupported MariaDB egress mode '{mode}'. Use replace, append, create, or create_append.")
    duck_conn = None
    duck_cur = None
    target_conn = None
    target_cur = None
    ...
    try:
        ...
        warnings.extend(
            _measure_mariadb_string_column_lengths(
                duck_conn,
                sql,
                column_meta,
        ...
        if callable(on_interrupt_open):
            target_interrupt_token = on_interrupt_open(_build_mariadb_interruptor(target_conn))
        target_cur = target_conn.cursor()
        if normalized_mode in {"replace", "create", "create_append"}:
            _ensure_mariadb_schema(target_cur, schema_name)
        table_exists = _mariadb_table_exists(target_cur, schema_name=schema_name, table_name=table_name)
        if normalized_mode == "replace":
            if table_exists:
                target_cur.execute(f"DROP TABLE {quoted_target_table}")
            create_sql, create_warnings = _build_mariadb_create_table_sql(normalized_target_table, column_meta)
        ...
        target_conn.commit()
        try:
            remote_columns = _inspect_mariadb_table_columns(target_cur, schema_name=schema_name, table_name=table_name)
            ...
            warnings.append(f"MariaDB remote schema inspection failed; using inferred egress column mappings. {exc}")
    finally:
        try:
...
def drop_table_if_exists(*, target_request: MariaDbConnectRequest, target_table: str) -> None:
    conn = None
    cur = None
    schema_name, table_name, normalized_target_table = _normalize_target_relation(target_table)
    quoted_target_table = _quote_compound_identifier(normalized_target_table)
    try:
        ...
        cur = conn.cursor()
        if _mariadb_table_exists(cur, schema_name=schema_name, table_name=table_name):
